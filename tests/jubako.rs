...
use mock::{crc32, CheckInfo, Cluster, Entry as TestEntry, Index, IndexStore, KeyStore, PackInfo};
...
#[fixture]
fn Compression() -> jubako::CompressionType {
    jubako::CompressionType::None
}
...
fn create_content_pack(
    ...
    pack_header_data.write_all(&[0x6a, 0x62, 0x6b, 0x63, 0x00, 0x00, 0x00, 0x01, 0x00, 0x02])?;
    let uuid = Uuid::from_slice(&[
        0xC6, 0x50, 0xD8, 0x3D, 0x80, 0x33, 0x44, 0xDF, 0xA2, 0xCD, 0x30, 0xAE, 0xEC, 0x84, 0x76,
        0xA7,
    ])
    .unwrap();
...
fn create_directory_pack(entries: &Vec<TestEntry>) -> Result<PackInfo> {
    ...
    // Pack header
    let mut pack_header_data = vec![];
    pack_header_data.write_all(&[0x6a, 0x62, 0x6b, 0x64, 0x00, 0x00, 0x00, 0x01, 0x00, 0x02])?;
    let uuid = Uuid::from_slice(&[
        0x24, 0x5F, 0x7B, 0x1D, 0xF0, 0x4E, 0x42, 0x2E, 0x99, 0x9D, 0xE3, 0x37, 0x15, 0x62, 0x68,
        0x92,
    ])
    .unwrap();
    pack_header_data.write_all(uuid.as_bytes())?;
...
fn create_main_pack(directory_pack: PackInfo, content_pack: PackInfo) -> Result<PathBuf> {
    ...
    let uuid = Uuid::from_slice(&[
        0x3E, 0xED, 0x3E, 0xED, 0xBA, 0x54, 0xFB, 0xC6, 0x44, 0x4A, 0x97, 0x23, 0x68, 0x8F, 0x01,
        0x5F,
    ])
    .unwrap();
    ...
    // Pack header
    file.write_all(&[0x6a, 0x62, 0x6b, 0x6d, 0x00, 0x00, 0x00, 0x01, 0x00, 0x02])?;
...
#[test]
fn test_content_pack(compression: Compression, articles: Articles) {
    let content_info = create_content_pack(*compression, &articles).unwrap();
    let directory_info = create_directory_pack(&articles).unwrap();
    ...
    let index = container
        .get_index_for_name("Super index")
        .unwrap()
        .expect("'Super index' is in the container");
    ...
    for i in index.count() {
        ...
        assert_eq!(vec, articles[i.into_u32() as usize].path.as_bytes());
        let value_1 = entry.get_value("V1").unwrap().unwrap();
        if let reader::RawValue::Content(content) = value_1 {
            assert_eq!(
                content,
                jubako::ContentAddress {
                    pack_id: 1.into(),
                    content_id: jubako::ContentIdx::from(i.into_u32())
                }
            );
        ...
        if let reader::RawValue::U16(v) = value_2 {
            assert_eq!(v, articles[i.into_u32() as usize].word_count);
        } else {
            panic!();
        }
    }
